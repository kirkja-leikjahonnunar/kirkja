...
@export_range(0.1, 100.0, 0.1) var stiffness : float = 1.0
@export_range(0.0, 100.0, 0.1) var damping: float = 0.0
@export var use_gravity := false
@export var gravity := Vector3(0, -9.81, 0)
@export_enum(X_Plus, Y_Plus, Z_Plus, X_Minus, Y_Minus, Z_Minus) var forward_axis : int = Axis.Z_Minus
...
func _process(delta):
	...
	if is_nan(bone_new_transf_obj.basis.x.x):
